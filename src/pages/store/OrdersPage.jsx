...
const OrdersPage = () => {
  ...
  const statusMap = {
    pending: { label: 'Chờ xử lý', color: 'text-amber-600 bg-amber-50', icon: <Clock size={14} /> },
    shipped: { label: 'Đang giao', color: 'text-blue-600 bg-blue-50', icon: <Truck size={14} /> },
    delivered: { label: 'Đã giao', color: 'text-green-600 bg-green-50', icon: <CheckCircle size={14} /> },
  ...
  if (loading) return <div className="flex justify-center items-center h-screen animate-pulse text-gray-400 font-medium">Đang tải đơn hàng...</div>;
    ...
    <div className="bg-gray-50 min-h-screen pb-12">
      ...
      <div className="bg-white border-b sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 h-16 flex items-center gap-4">
          <Link to="/market" className="hover:bg-gray-100 p-2 rounded-full transition"><ArrowLeft size={20} /></Link>
      ...
      <div className="bg-white border-b sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link to="/market" className="p-2 hover:bg-gray-100 rounded-full transition">
      ...
      <div className="max-w-4xl mx-auto px-4 mt-6">
        {/* Tabs */}
        <div className="flex bg-white rounded-xl shadow-sm mb-6 overflow-hidden border">
          {['all', 'pending', 'shipped', 'delivered'].map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-1 py-4 text-sm font-bold transition-all ${
                activeTab === tab ? 'text-orange-600 border-b-2 border-orange-600 bg-orange-50/30' : 'text-gray-500 hover:bg-gray-50'
              }`}
            >
              ...
              <div key={order.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition">
                ...
                <div className="px-6 py-4 border-b flex justify-between items-center bg-gray-50/50">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-xs font-bold text-gray-400">#{order.id.slice(0, 10).toUpperCase()}</span>
                  ...
                  {order.items?.map((item, idx) => (
                    <div key={idx} className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 mt-1">Phân loại: {item.category || 'Mặc định'}</p>
                        <div className="flex justify-between items-center mt-2">
                          <span className="text-xs font-medium text-gray-600 uppercase italic">Số lượng: {item.quantity}</span>
                ...
                {/* Order Footer */}
                <div className="px-6 py-4 bg-orange-50/20 border-t flex flex-col sm:flex-row justify-between items-center gap-4">
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-600">Thành tiền:</span>
                    <span className="text-xl font-black text-orange-600">${order.total?.toLocaleString()}</span>
                  </div>
                  <div className="flex gap-2 w-full sm:w-auto">
                    <button className="flex-1 sm:flex-none px-5 py-2 text-xs font-bold text-gray-600 bg-white border rounded-lg hover:bg-gray-50 transition">
                      Xem chi tiết
                    </button>
